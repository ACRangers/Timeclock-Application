...
**A manual task spans the time it took**, so it needs its own strip of the column — the left 36%, with the activity lane taking the rest and reclaiming the full width on a day with no tasks. Sharing the activity lane would mean a two-hour task lying across every call underneath it. `packColumns()` divides the strip between tasks that overlap each other. Reads `Category · Title · time · Job #`, coloured by category: Admin green, Meeting red, Others yellow.
